...
export async function POST(request: NextRequest): Promise<NextResponse> {
    ...
    try {
        ...
        const { spotify, youtube, website, additional } = await request.json();
        if (!tokenId) {
        ...
        user.otherLink = additional
        user = await user.save()
        console.log("USER CREDENTIALS AFTER UPDATE SOCIAL LINKS:", user) //TODO: REMOVE 
        ...
        response = {
            success: true,
            message: "User Social Links successfully",
            statusCode: 201,
            data: user
        }
